...
import type { PoderClasseJson } from "@/lib/t20/jsonTypes";
...
export function ClassListBlock({ sheet, onChange }: ClassListBlockProps) {
  ...
  const [modalClassIndex, setModalClassIndex] = useState<number | null>(null);
  ...
  return (
    <section className="space-y-5 rounded-md border border-border bg-paper-card p-5 shadow-sm">
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => setModalClassIndex(index)}
                    className="rounded border border-border bg-paper-card px-2 py-1 text-xs font-medium text-ink hover:bg-paper"
                  >
                    Ver detalhes
                  </button>
                  <button
      ...
      {modalClassIndex !== null && classes[modalClassIndex] && (() => {
        const klass = classes[modalClassIndex!];
        ...
        return (
          <div
            ...
            onClick={() => setModalClassIndex(null)}
            role="dialog"
            aria-modal="true"
            aria-labelledby="modal-classe-titulo"
          >
            <div
            ...
            >
              ...
              <div className="min-h-0 flex-1 space-y-3 overflow-y-auto text-xs text-ink-muted">
                    ...
                    {poderes.map((p) => (
                      <li key={p.id}>
                        ...
                        {p.descricao_resumida
                          ? ` — ${p.descricao_resumida}`
                          : ""}
                        {p.pode_escolher_multiplas_vezes
